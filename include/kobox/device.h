...
typedef struct kb_device_backend kb_device_backend_t;
typedef struct kb_device kb_device_t;
typedef struct kb_device_irq kb_device_irq_t;
...
typedef struct kb_device_backend_ops {
    ...
    kb_status_t (*device_count)(kb_device_backend_t *backend, size_t *out_count);
    kb_status_t (*device_at)(kb_device_backend_t *backend, size_t index, kb_device_t **out_device);
    ...
    void (*irq_unregister)(kb_device_t *device, kb_device_irq_t *irq);
    kb_status_t (*irq_wait)(kb_device_t *device, kb_device_irq_t *irq, uint64_t timeout_ns);
