...
static kb_linux_sysfs_backend_t *sysfs_from_backend(kb_device_backend_t *backend)
{
...
static void sysfs_destroy(kb_device_backend_t *backend)
{
...
static kb_status_t sysfs_device_count(kb_device_backend_t *backend, size_t *out_count)
{
...
static kb_status_t sysfs_device_at(kb_device_backend_t *backend, size_t index, kb_device_t **out_device)
{
...
static void sysfs_irq_unregister(kb_device_t *device, kb_device_irq_t *irq)
{
    (void)device;
    (void)irq;
}

static kb_status_t sysfs_irq_wait(kb_device_t *device, kb_device_irq_t *irq, uint64_t timeout_ns)
{
    (void)device;
    (void)irq;
    (void)timeout_ns;
...
static uint64_t sysfs_monotonic_ns(kb_device_backend_t *backend)
{
    (void)backend;
    struct timespec ts;
...
static void sysfs_log(kb_device_backend_t *backend, int level, const char *message)
{
    (void)backend;
    (void)level;
    (void)message;
...
static const kb_device_backend_ops_t sysfs_ops = {
