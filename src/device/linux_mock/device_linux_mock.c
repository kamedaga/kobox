...
static kb_linux_mock_backend_t *mock_from_backend(kb_device_backend_t *backend)
{
...
static kb_status_t mock_device_count(kb_device_backend_t *backend, size_t *out_count)
{
    (void)backend;
    if (out_count == 0) {
...
static kb_status_t mock_device_at(kb_device_backend_t *backend, size_t index, kb_device_t **out_device)
{
...
static kb_status_t mock_map_bar(kb_device_t *device, unsigned bar_index, kb_mmio_region_t *out_region)
{
    ...
    out_region->host_phys = device->bar_starts[bar_index];
...
static void mock_irq_unregister(kb_device_t *device, kb_device_irq_t *irq)
{
    (void)device;
    free(irq);
}

static kb_status_t mock_irq_wait(kb_device_t *device, kb_device_irq_t *irq, uint64_t timeout_ns)
{
    (void)device;
    (void)timeout_ns;
...
static uint64_t mock_monotonic_ns(kb_device_backend_t *backend)
{
    (void)backend;
    struct timespec ts;
...
static void mock_log(kb_device_backend_t *backend, int level, const char *message)
{
    (void)backend;
    (void)level;
    (void)message;
...
kb_status_t kb_linux_mock_device_create(kb_device_backend_t **out_backend)
{
    if (out_backend == 0) {
        return KB_ERR_INVALID;
    }
