...
static int remember_mmio_mapping(
    ...
    int bar)
{
    if (backend == NULL || device == NULL || region == NULL || region->addr == NULL || region->size == 0 ||
        user_addr == NULL) {
        return -22;
    }
...
static shim_mmio_mapping_t *retain_mmio_mapping_for_bar(kb_device_backend_t *backend, kb_device_t *device, int bar)
{
...
static shim_mmio_mapping_t *find_mmio_mapping_for_bar(kb_device_backend_t *backend, kb_device_t *device, int bar)
{
    if (backend == NULL || device == NULL) {
        return NULL;
    }
