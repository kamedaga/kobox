...
if [ -z "$nvidia_module" ] || [ ! -f "$nvidia_module" ]; then
    echo "skip nvidia drm mock smoke: nvidia.ko was not found under .artifacts/nvidia-*" >&2
    exit 0
fi

if [ -z "$modeset_module" ] || [ ! -f "$modeset_module" ]; then
    echo "skip nvidia drm mock smoke: nvidia-modeset.ko was not found under .artifacts/nvidia-*" >&2
    exit 0
fi

if [ -z "$drm_module" ] || [ ! -f "$drm_module" ]; then
    echo "skip nvidia drm mock smoke: nvidia-drm.ko was not found under .artifacts/nvidia-*" >&2
    exit 0
fi

log="$repo_root/.artifacts/nvidia-drm-mock-smoke.log"
...
if grep -q "probe routine was not called" "$log"; then
    echo "nvidia drm mock smoke: unexpected probe-count warning" >&2
    exit 1
fi
echo "nvidia drm mock smoke: $(tail -n 1 "$log")"
