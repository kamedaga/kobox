...
work_dir="$repo_root/.artifacts/qemu-vfio-nvme-ext4"
...
echo "kobox-qemu-vfio-nvme-ext4: booted"
...
if [ -z "$nvme_bdf" ]; then
    echo "kobox-qemu-vfio-nvme-ext4: nvme device not found"
    poweroff -f
fi
...
vendor=${vendor#0x}
device=${device#0x}
echo "kobox-qemu-vfio-nvme-ext4: nvme=$nvme_bdf id=$vendor:$device"
...
echo "kobox-qemu-vfio-nvme-ext4: group=$group"
set +e
...
status=$?
set -e
echo "kobox-qemu-vfio-nvme-ext4: status=$status"
poweroff -f
INIT
...
grep -q "kobox-qemu-vfio-nvme-ext4: status=0" "$serial_log"
